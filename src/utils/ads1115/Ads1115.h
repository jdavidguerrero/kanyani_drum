...
// Constantes de configuración del ADS1115
...
/**************************************************************************/
/*!
    @brief  Sensor driver for the RPI ADS1X15 ADC breakouts.
*/
/**************************************************************************/
// Clase base para el ADS1X15
class ADS1X15
{
protected:
    int m_i2c_handle;
...
public:
    /**
     * @brief Inicializa el ADS1X15.
     * @param i2c_addr Dirección I2C del dispositivo.
     * @return True si la inicialización es exitosa, False en caso contrario.
     */
    virtual bool begin(uint8_t i2c_addr);

    /**
     * @brief Realiza una lectura de un solo canal en modo single-ended.
     * @param channel Canal a leer (0-3).
     * @return Valor leído del ADC.
     */
    int16_t readADC_SingleEnded(uint8_t channel);

    /**
     * @brief Configura el comparador en modo single-ended.
     * @param channel Canal a utilizar (0-3).
     * @param threshold Umbral del comparador.
     */
    void startComparator_SingleEnded(uint8_t channel, int16_t threshold);

    /**
     * @brief Obtiene el último resultado de conversión.
     * @return Último resultado de conversión.
     */
    int16_t getLastConversionResults();

    /**
     * @brief Calcula el voltaje a partir de las cuentas del ADC.
     * @param counts Cuentas del ADC.
     * @return Voltaje calculado.
     */
    float computeVolts(int16_t counts);

    /**
     * @brief Configura la ganancia del ADC.
     * @param gain Ganancia a configurar.
     */
    void setGain(adsGain_t gain);

    /**
     * @brief Obtiene la ganancia actual del ADC.
     * @return Ganancia actual.
     */
    adsGain_t getGain();

    /**
     * @brief Configura la tasa de datos del ADC.
     * @param rate Tasa de datos a configurar.
     */
    void setDataRate(uint16_t rate);

    /**
     * @brief Obtiene la tasa de datos actual del ADC.
     * @return Tasa de datos actual.
     */
    uint16_t getDataRate();

protected:
    /**
     * @brief Escribe un valor en un registro del ADS1X15.
    ...
    /**
     * @brief Lee un valor de un registro del ADS1X15.
    ...
    /**
     * @brief Inicia una lectura del ADC.
     * @param mux Configuración de multiplexación.
     * @param continuous Modo de conversión continua.
     */
    void startADCReading(uint16_t mux, bool continuous);

    /**
     * @brief Verifica si la conversión del ADC está completa.
     * @return True si la conversión está completa, False en caso contrario.
     */
    bool conversionComplete();

    void startContinuousReading(uint8_t channel);

private:
    std::mutex m_mutex;
};

// Clase específica para el primer ADS1115
class ADS1115_0 : public ADS1X15
{
public:
    ADS1115_0();
};

// Clase específica para el segundo ADS1115
class ADS1115_1 : public ADS1X15
{
public:
    ADS1115_1();
};

// Clase específica para el ADS1115
class ControllerADS1115
{
private:
    ADS1115_0 *m_ads1115_0;
    ADS1115_1 *m_ads1115_1;
...
public:
    ...
    /**
     * @brief Realiza una lectura de un solo canal en modo single-ended.
     * @param device Dispositivo ADS1115 (0 o 1).
     * @param channel Canal a leer (0-3).
     * @return Valor leído del ADC.
     */
    int16_t readADC_SingleEnded(uint8_t device, uint8_t channel);
};
